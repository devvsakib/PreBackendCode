...
const MovieCard = ({ drama, idx, addDrama }) => {
    ...
    useEffect(() => {
        ...
        axios.get(api)
            ...
            console.log(JSON.parse(localStorage.getItem('drama')));
    }, [drama])

    useEffect(()=>{

        axios.post('http://localhost:5000/add', {name, thumbnail, description, watched})
            ...
            <div key={idx} className="card flex-col  shadow-lg card-side  p-0 overflow-hidden bg-base-100" >
                ...
                <div className="text-left p-3">
                    <div className="flex gap-5">
                        ...
                        {/* Add */}
                        <button onClick={()=>{setName(singleDrama.title); setDescription(synopsis); setThumbnail(singleDrama.poster);setWatched(false)}} className="my-2 btn-primary px-4 rounded-md uppercase font-semibold">Add</button>
                    </div>
